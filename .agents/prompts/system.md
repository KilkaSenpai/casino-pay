...
- Split a multi-section UI block the same way as `Header/`: `Site*` fetches CMS data, then one file per design section (nav, logos, legal). Do not put a whole page block in one file. Do not extract a 20-line button into its own file.
